...
static bool CollectAudioSource(void *parameter,void *source){
    auto *collection=static_cast<AudioSourceCollection*>(parameter);constexpr uint32_t AUDIO_FLAG=1u<<1;if(!source||(api.source_output_flags(source)&AUDIO_FLAG)==0||!api.source_audio_active(source))return true;
    bool active=api.source_active(source);if(collection->activeOnly&&!active)return true;
    void *reference=api.source_get_ref(source);if(!reference)return true;const char *rawName=api.source_name(reference);QString name=QString::fromUtf8(rawName?rawName:"");if(name.isEmpty())name=LText(LocalText::AudioSource);VolumeEntry entry;entry.source=reference;entry.name=std::move(name);entry.active=active;collection->entries->push_back(std::move(entry));return true;
}
...
static void VolumeConsoleHotkey(void*,hotkey_id,obs_hotkey*,bool pressed){
    if(!pressed||!obsMainWindow||shuttingDown)return;QMetaObject::invokeMethod(obsMainWindow,[]{if(shuttingDown)return;if(volumeConsoleWindow){if(volumeConsoleWindow->isMinimized())volumeConsoleWindow->showNormal();else if(!volumeConsoleWindow->isVisible())volumeConsoleWindow->show();if(QApplication::activeWindow()!=volumeConsoleWindow){volumeConsoleWindow->raise();volumeConsoleWindow->activateWindow();}return;}if(!MainInterfaceActive())return;volumeConsoleReturnFocus=QApplication::focusWidget();auto *dialog=new VolumeConsoleDialog(obsMainWindow);volumeConsoleWindow=dialog;QObject::connect(dialog,&QObject::destroyed,obsMainWindow,[]{volumeConsoleWindow=nullptr;if(shuttingDown){volumeConsoleReturnFocus=nullptr;return;}QPointer<QWidget> returnFocus=volumeConsoleReturnFocus;volumeConsoleReturnFocus=nullptr;if(returnFocus&&returnFocus->isVisible()&&returnFocus->isEnabled()&&QApplication::focusWidget()!=returnFocus){if(QWidget *window=returnFocus->window();window&&QApplication::activeWindow()!=window)window->activateWindow();returnFocus->setFocus(Qt::ShortcutFocusReason);}});dialog->open();dialog->raise();dialog->activateWindow();},Qt::QueuedConnection);
}
