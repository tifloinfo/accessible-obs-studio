...
static OpenAIResult CallOpenAI(const std::vector<uint8_t> &pixels,uint32_t width,uint32_t height,int format,std::string apiKey){
    ...
    QJsonArray content{QJsonObject{{QStringLiteral("type"),QStringLiteral("input_text")},{QStringLiteral("text"),QStringLiteral("Describe the supplied rendered video frame for the blind user.")}},imageContent};
...
static void QueueFollowup(OpenAIResult result){
    QMetaObject::invokeMethod(obsMainWindow,[result=std::move(result)]() mutable {requestRunning=false;if(shuttingDown)return;if(!descriptionTurns.empty()&&descriptionTurns.back().first==Tr(UiText::Status))descriptionTurns.pop_back();if(result.success){currentResponseId=std::move(result.responseId);if(result.text==L"ACCESSIBLE_OBS_OUT_OF_SCOPE")result.text=OutOfScopeText();descriptionTurns.emplace_back(Tr(UiText::OpenAIResponse),std::move(result.text));}else descriptionTurns.emplace_back(Tr(UiText::Error),std::move(result.text));ShowWindowPreservingState(descriptionWindow);RenderDescription(false,true);},Qt::QueuedConnection);
}
