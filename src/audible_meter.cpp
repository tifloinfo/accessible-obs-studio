...
static const AudibleText AM_INPUT_DIALOG_TEXT={"Sound quality is reduced for \"%1\" because its prefade signal is in the red. Do you wish to adjust prefade for this source?","Die Tonqualität von „%1“ ist beeinträchtigt, weil das Signal vor dem Regler im roten Bereich liegt. Möchten Sie den Pegel vor dem Regler für diese Quelle anpassen?","Качество звука «%1» снижено, поскольку сигнал до регулятора находится в красной зоне. Настроить уровень до регулятора для этого источника?","Якість звуку «%1» знижено, оскільки сигнал до регулятора перебуває в червоній зоні. Налаштувати рівень до регулятора для цього джерела?","La qualité sonore de « %1 » est réduite, car son signal avant curseur est dans le rouge. Voulez-vous régler le niveau avant curseur de cette source ?","La calidad del sonido de «%1» se ha reducido porque su señal antes del control está en rojo. ¿Desea ajustar el nivel anterior al control para esta fuente?"};
...
static void LoadWarningExposure(){
    warningExposureMs=AM_DEFAULT_WARNING_MS;config *cfg=api.global_config?api.global_config():nullptr;if(!cfg||!api.config_has_user_value(cfg,"AccessibleOBSStudio","AudibleMeterWarningSeconds"))return;double seconds=api.config_get_double(cfg,"AccessibleOBSStudio","AudibleMeterWarningSeconds");if(std::isfinite(seconds)&&seconds>=0.1&&seconds<=30.0)warningExposureMs=static_cast<int>(std::lround(seconds*1000.0));
}
...
static void UpdateAlert(AlertTracker &a,MeterZone zone,double elapsed){
    if(zone==MeterZone::Red){a.redMs+=elapsed;a.recoveryMs=0;if(a.armed&&a.redMs>=warningExposureMs.load()){a.pending=true;a.armed=false;}}
...
static void MeterCallback(void *p,const float *magnitude,const float *peak,const float *inputPeak){
    ...
    if(audibleWarningsEnabled.load()&&!consoleOpen.load()){if(s->checkPrefader&&!s->inputIgnored)UpdateAlert(s->inputAlert,s->inputZone,elapsed);UpdateAlert(s->outputAlert,s->outputZone,elapsed);}
}
...
static void UpdateAudibleOutput(){
    if(!audibleMeterActive.load()){SetTone(AudibleTone::None);return;}if(consoleOpen.load()){MeterZone z=FocusedZone();SetTone(z==MeterZone::Red?AudibleTone::RedMeasurement:(z==MeterZone::Yellow?AudibleTone::YellowMeasurement:AudibleTone::None));return;}auto now=std::chrono::steady_clock::now();bool inputActive=false,outputActive=false;double environmentDb=-INFINITY;std::vector<PendingAlert> input,output;
    if(audibleWarningsEnabled.load())for(const auto &s:meterSources){std::lock_guard<std::mutex> lock(s->mutex);bool fresh=s->updated.time_since_epoch().count()&&std::chrono::duration_cast<std::chrono::milliseconds>(now-s->updated).count()<=500;if(fresh&&std::isfinite(s->outputDb))environmentDb=std::max(environmentDb,s->outputDb);inputActive|=fresh&&s->checkPrefader&&!s->inputIgnored&&!s->inputAlert.armed&&s->inputZone==MeterZone::Red;outputActive|=fresh&&!s->outputAlert.armed&&s->outputZone==MeterZone::Red;if(s->inputAlert.pending){if(!s->checkPrefader||s->inputPromptShown)s->inputAlert.pending=false;else input.push_back({s->uuid,s->name,s->inputDb});}if(s->outputAlert.pending){output.push_back({s->uuid,s->name,s->outputDb});s->outputAlert.pending=false;}}
    if(inputActive)SetTone(AudibleTone::InputWarning,environmentDb);else if(outputActive)SetTone(AudibleTone::OutputWarning,environmentDb);else SetTone(AudibleTone::None);
...
static bool SaveAudibleMeterWarningSeconds(double seconds){
    if(!std::isfinite(seconds)||seconds<0.1||seconds>30.0)return false;config *cfg=api.global_config?api.global_config():nullptr;if(!cfg)return false;api.config_set_double(cfg,"AccessibleOBSStudio","AudibleMeterWarningSeconds",seconds);if(api.config_save_safe(cfg,"tmp",nullptr)!=0)return false;warningExposureMs=static_cast<int>(std::lround(seconds*1000.0));ResetAllAlerts();UpdateAudibleOutput();return true;
}
...
static void InitializeAudibleMeter(){LoadWarningExposure();for(int i=0;i<AM_BINS;++i)binEnergy[static_cast<size_t>(i)]=std::pow(10.0,(i+AM_MIN_DB)/10.0);qApp->installEventFilter(new AudibleMeterKeyFilter(pluginEventContext));}
static void AudibleMeterConsoleOpened(){consoleOpen=true;ResetAllAlerts();UpdateAudibleOutput();}
static void AudibleMeterConsoleClosed(){consoleOpen=false;consoleFocusedUuid.clear();ResetAllAlerts();UpdateAudibleOutput();}
