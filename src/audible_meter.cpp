...
static void MeterCallback(void *p,const float *magnitude,const float *peak,const float *inputPeak){
    ...
    if(audibleWarningsEnabled.load()){UpdateAlert(s->inputAlert,s->inputZone,elapsed);UpdateAlert(s->outputAlert,s->outputZone,elapsed);}
}
...
static bool Attach(MeterSource &s,void *enumerated){
    ...
    if(!api.volmeter_attach(m,ref)){api.volmeter_remove_callback(m,MeterCallback,&s);api.volmeter_destroy(m);api.source_release(ref);return false;}s.source=ref;s.meter=m;s.channels=std::clamp(api.volmeter_channels(m),1,AM_MAX_CHANNELS);return true;
}
...
static bool Collect(void *p,void *enumerated){
    ...
    auto it=std::find_if(meterSources.begin(),meterSources.end(),[&](const auto &s){return s->uuid==id;});if(it!=meterSources.end()){std::lock_guard<std::mutex> lock((*it)->mutex);(*it)->name=name;return true;}
