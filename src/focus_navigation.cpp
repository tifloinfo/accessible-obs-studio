...
static void FinishFitQualityValidation(bool acceptable){
    if(acceptable){pendingFitSource=QPersistentModelIndex();QMessageBox::information(obsMainWindow,QStringLiteral("Accessible OBS Studio"),LText(LocalText::FitQualityAccepted)+QStringLiteral("\n\n")+LText(LocalText::Undo));return;}
    QAction *undo=obsMainWindow?obsMainWindow->findChild<QAction*>(QStringLiteral("actionMainUndo")):nullptr;if(!undo||!undo->isEnabled()){pendingFitSource=QPersistentModelIndex();QMessageBox::warning(obsMainWindow,QStringLiteral("Accessible OBS Studio"),LText(LocalText::FitUndoFailed));return;}undo->trigger();RestorePendingFitSelection();
    QAction *center=obsMainWindow->findChild<QAction*>(QStringLiteral("actionCenterToScreen"));if(!center||!center->isEnabled()){pendingFitSource=QPersistentModelIndex();QMessageBox::warning(obsMainWindow,QStringLiteral("Accessible OBS Studio"),LText(LocalText::FitCenterFailed));return;}center->trigger();pendingFitSource=QPersistentModelIndex();QMessageBox::information(obsMainWindow,QStringLiteral("Accessible OBS Studio"),LText(LocalText::FitQualityFallback)+QStringLiteral("\n\n")+LText(LocalText::Undo));
}
...
static constexpr const char *ACCESSIBLE_OBS_BUILD_ID="1.1.0-audible-meter-test-20260730-4";
...
static void ReviewKeyboardShortcutConflicts(){
    config *profile=api.profile_config?api.profile_config():nullptr;if(!profile)return;LoadAccessibilityBindings();const char *reviewed=api.config_get_string(profile,"AccessibleOBSStudio","ShortcutReviewBuild"),*schema=api.config_get_string(profile,"AccessibleOBSStudio","ShortcutDefaultsSchema");bool initializeDefaults=!schema||strcmp(schema,SHORTCUT_DEFAULTS_SCHEMA)!=0;if(reviewed&&strcmp(reviewed,ACCESSIBLE_OBS_BUILD_ID)==0&&!initializeDefaults)return;HotkeyCollector collector;api.enum_hotkeys(CollectHotkey,&collector);api.enum_bindings(CollectBinding,&collector);auto defaults=AccessibilityDefaults();std::vector<PlannedShortcut> plans;
    ...
    api.config_set_string(profile,"AccessibleOBSStudio","ShortcutDefaultsSchema",SHORTCUT_DEFAULTS_SCHEMA);api.config_set_string(profile,"AccessibleOBSStudio","ShortcutReviewBuild",ACCESSIBLE_OBS_BUILD_ID);api.config_save_safe(profile,"tmp",nullptr);if(choice.suppress&&global){api.config_set_string(global,"AccessibleOBSStudio","ShortcutPromptSuppressedBuild",ACCESSIBLE_OBS_BUILD_ID);api.config_set_string(global,"AccessibleOBSStudio","ShortcutConflictPolicy",choice.policy==ConflictPolicy::Replace?"replace":"keep");api.config_save_safe(global,"tmp",nullptr);}
}
...
static void FrontendEvent(int event,void*){constexpr int PROFILE_CHANGED=15,FINISHED_LOADING=26;if(event==PROFILE_CHANGED||event==FINISHED_LOADING)QueueProfileReview();HandleAccessibilityFrontendEvent(event);HandleAudibleMeterFrontendEvent(event);}
