...
static bool LoadNavigationBinding(hotkey_id id,const char *name,const char *defaultKey,uint32_t modifiers,bool persistMissing,int virtualKey=0){
    ...
    int key=virtualKey&&api.key_from_virtual_key?api.key_from_virtual_key(virtualKey):api.key_from_name(defaultKey);key_combo combo{modifiers,key};api.load_bindings(id,&combo,1);if(persistMissing)SaveNavigationBinding(cfg,id,name);return persistMissing&&cfg;
}
