...
$allSource = $plugin + $audibleMeter + $focusNavigation + $volumeConsole + $canvas + $installer
...
Assert-True ($audibleMeter -match 'AM_WARNING_MS=1000') 'Audible Meter does not use the fixed 1-second warning exposure.'
Assert-True ($audibleMeter -match 'std::try_to_lock') 'The audio callback can block on UI state.'
Assert-True ($audibleMeter -notmatch '(?i)report|history|QTreeWidget|QDialog|QSaveFile') 'Removed Diagnostic report or history code is still present.'
...
Assert-True ($audibleMeter -match 'else if\(consoleOpen\)') 'Focused-source meter tones are not lower priority than warnings.'
...
Assert-True (($audibleMeter -match 'COLLECTION_CHANGING=35') -and ($audibleMeter -match 'collectionSwitchInProgress=true;SetTone\(AudibleTone::None\);SuspendMeters\(\)')) 'Audible Meter does not release source references before a scene collection unload.'
...
Assert-True ($installer -match '1\.1\.0-test\.2') 'The installer is not clearly identified as the current test build.'
