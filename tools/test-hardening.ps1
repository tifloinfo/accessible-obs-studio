...
Assert-True ($audibleMeter -match 'AM_WARNING_MS=1500') 'Audible Meter does not use the fixed 1.5-second warning exposure.'
...
Assert-True ($audibleMeter -match 'if\(inputActive\)SetTone\(AudibleTone::InputWarning\);else if\(outputActive\)') 'Input danger does not take priority over output warnings.'
