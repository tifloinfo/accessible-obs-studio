...
Assert-True (($audibleMeter -match 'AM_INPUT_DIALOG_TEXT') -and ($audibleMeter -match 'setDefaultButton\(QMessageBox::Yes\)') -and ($audibleMeter -match 'setEscapeButton\(QMessageBox::No\)')) 'The pre-fader warning decision dialog is incomplete.'
...
Assert-True ($installer -match '1\.1\.0-test\.3') 'The installer is not clearly identified as the current test build.'
