...
Assert-True (($qtInterface -match 'AudibleMeterSettingsDialog') -and ($qtInterface -match 'OutputWarningLevel') -and ($qtInterface -match 'TimeAboveWarningLevel') -and ($qtInterface -match 'PeakSafetyMargin') -and ($qtInterface -match 'MaximumAutomaticReduction') -and ($qtInterface -notmatch 'QDoubleSpinBox \*warningDelay')) 'The dedicated Audible Meter settings dialog is incomplete.'
...
Assert-True (($soundDoctor -match 'MONITOR_SECONDS=120') -and ($soundDoctor -match 'MIN_ACTIVE_MS=45000\.0') -and ($soundDoctor -match 'std::try_to_lock')) 'Sound Doctor monitoring is not bounded or its callback can block.'
...
Assert-True (($qtInterface -notmatch 'audibleSettings=buttons->addButton') -and ($qtInterface -notmatch 'apiSettings=buttons->addButton') -and ($qtInterface -match 'AudibleMeterSettingsDialog') -and ($qtInterface -match 'ShowSettings\(\)')) 'Settings commands were not removed from the Shortcut Editor while retaining their menu dialogs.'
