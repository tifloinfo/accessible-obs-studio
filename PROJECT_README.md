...
Ctrl+Grave opens the modal **Accessible Volume Console** without changing the control remembered in the main OBS window. The console initially shows only audio sources active in the current program output; Preview-only sources in Studio Mode are excluded. Activate **Show All Sources** with Enter to include every source currently available in the OBS Mixer; the same non-default button then becomes **Show Active Sources Only**. Space does not activate this view button. Active sources remain first in the all-sources view. Left and Right move between sources; Up and Down change the selected source by 1 dB; Home restores 0 dB; Space safely toggles monitoring and program output together; Ctrl+Space toggles monitoring only; and Shift+Space toggles program output only. Each source also has separate output and monitoring buttons. Number keys 1 through 9 select the first nine sources and 0 selects the tenth. Changes take effect immediately. Escape closes the console and restores the previous OBS control. JAWS and NVDA receive the source name, volume in dB, output state, and monitoring state as values change.
...
Press Ctrl+Shift+P before a stream to start **Peak Guard Sound Check**. Its history window opens in the background so focus remains in OBS; use Alt+Tab if you want to inspect it. Sound Check monitors every audio source currently available to OBS and automatically includes audio sources added while it runs. Exercise the planned scenes, microphones, media, and other sound sources. A quiet continuous higher tone warns about dangerous output, while a lower tone warns about dangerous input before the OBS source fader. Input warnings take priority when both conditions occur. The Accessible Volume Console opens with focus on the most severe affected source. Lower that source until the tone stops; if other sources remain dangerous, focus advances to the next one. Peak conditions are also appended to a read-only, scrollable history control. The tones and automatic volume-console focus are used only during Sound Check. **Save History** writes a text copy on demand; automatic text and JSON histories are retained when Sound Check is accepted.

Press Ctrl+Shift+P again, or activate **Finish Sound Check**, to close the history window and open **Peak Guard Select Sources**. Select sources with Space and press Enter on the default OK button to start silent **Emergency Monitoring**. Cancel or Escape discards the current Sound Check history and returns Peak Guard to Idle. Peak Guard verifies an enabled OBS Limiter filter for every selected source. It honors an enabled limiter already supplied by the user; otherwise, it creates or re-enables its own limiter with the configured threshold and release time. Peak Guard-owned limiters remain in the source filter chain after monitoring stops or OBS closes. A later Sound Check temporarily bypasses only those Peak Guard-owned limiters so the underlying setup can be tested again, then restores them. Press Ctrl+Shift+P during Emergency Monitoring to stop monitoring without removing any limiter.
...
The **Settings** button in Sound Check opens the accessible Peak Guard Settings editor. Settings are stored in `%APPDATA%\obs-studio\plugin_config\accessible-obs-studio\peak-guard.json`. The same directory contains `peak-guard.defaults.json`, the documented `peak-guard.schema.json`, and the `peak-guard-history` directory. The JSON can be edited directly while Peak Guard is stopped, but the Settings editor validates values and writes the file safely.
