...
Ctrl+Grave opens the modal **Accessible Volume Console** without changing the control remembered in the main OBS window. **Active Sources Only** is checked whenever the console opens, so only audio sources active in the current program output are shown; Preview-only sources in Studio Mode are excluded. Clear the checkbox to include every source currently available in the OBS Mixer. Active sources remain first in that view. Left and Right move between sources; Up and Down change the selected source by 1 dB; Home restores 0 dB; Space safely toggles monitoring and program output together; Ctrl+Space toggles monitoring only; and Shift+Space toggles program output only. Each source also has separate output and monitoring buttons. Number keys 1 through 9 select the first nine sources and 0 selects the tenth. Changes take effect immediately. Escape closes the console and restores the previous OBS control. JAWS and NVDA receive the source name, volume in dB, output state, and monitoring state as values change.
